...
k8s_resource(
  ...
  new_name='database',
  objects=['developersbay-postgres-cluster'],
  extra_pod_selectors=[{'cnpg.io/cluster': 'developersbay-postgres-cluster'}],
  port_forwards=5432)
...
deployment_create(
  ...
  envFrom=[{
    "secretRef":{
      "name": "developersbay-postgres-cluster-app" # autogenerated when creating a postgres cluster without any initdb arguments
    }
  }]
  )
k8s_resource(
